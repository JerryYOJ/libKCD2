...
{
    // <game>/Mods/TruffleHoundMutt/KCSE/Plugins/TruffleHoundMutt.dll -> <game>/Mods/TruffleHoundMutt/mod.cfg
    HMODULE hm = nullptr;
...
void WriteCfg()
{
    ...
    if (!f)
        return;
    f << "-- TruffleHoundMutt settings - auto-generated from the MCM menu; the game applies\n"
         "-- this file at startup.  Delete a line to return that setting to its default.\n";
...
void PersistSetting(const char* cvarName, double value, bool isFloat)
{
    ...
    if (it != g_entries.end() && it->second == text)
        return;   // unchanged (e.g. a toggle re-set to the stored value)
