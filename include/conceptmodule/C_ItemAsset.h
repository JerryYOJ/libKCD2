...
// -----------------------------------------------
// wh::conceptmodule::C_ItemAsset : C_BaseAsset
...
class C_ItemAsset : public C_BaseAsset {
public:
    ~C_ItemAsset() override;                                   // [0]
    RTTR_ENABLE(C_BaseAsset)                              // [2..4]
    std::uint8_t m_unknown18[0x18];                       // +0x18 ItemClassGuids accessor-backed; offset unproven
};

static_assert(offsetof(C_ItemAsset, m_unknown18) == 0x18,
              "C_ItemAsset::m_unknown18 offset mismatch");
static_assert(sizeof(C_ItemAsset) == 0x30,
              "C_ItemAsset size mismatch");
