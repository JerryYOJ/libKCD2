...
public:
    ...
    bool m_skipDefenseZoneRestore; // +0x88  bool; dtor-path slot[58] skips a reset (context+336) when set
    bool m_skipOpponentAttackZoneClear; // +0x89  bool; dtor-path slot[58] skips a reset (context+1488=-1) when set; setters 0x181A71900/0x181A71910 write 0/1
    uint16_t _pad8A;               // +0x8A
    uint32_t m_actionInfo0; // +0x8C  (init 0)  dword copied to action-info out[0] by slot[35] 0x1809E37FF; role UNVERIFIED
    uint32_t m_actionInfo1; // +0x90  (init 0)  dword copied to action-info out[4] and tested (if(!field90)) by slot[35] 0x1809E380A/0x1809E381E; role UNVERIFIED
    bool     m_field94;            // +0x94  bool init 1; virtual getter slot[30] 0x181A7D990 returns this byte
    bool m_inGuardStateApply; // +0x95
