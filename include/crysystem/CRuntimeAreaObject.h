...
#include "SRuntimeAreaSurfaceAudioState.h"
...
class CRuntimeAreaObject
    : public Offsets::CGameObjectExtensionHelper<
          ...
          64> {
public:
    inline static constexpr auto RTTI = Offsets::RTTI_CRuntimeAreaObject;

    CRuntimeAreaObject();
    ~CRuntimeAreaObject() override; // [0]
    ...
    std::map<EntityId,
             std::map<std::uint32_t, SRuntimeAreaSurfaceAudioState>>
        m_entityAudioStates; // +0x40
};
static_assert(sizeof(CRuntimeAreaObject) == 0x50,
              "CRuntimeAreaObject must be 0x50");
static_assert(offsetof(CRuntimeAreaObject, m_entityAudioStates) == 0x40,
              "runtime-area audio state map must be at 0x40");
