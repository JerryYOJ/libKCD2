...
class C_DialogueHolder
    : public Offsets::CGameObjectExtensionHelper<
          C_DialogueHolder,
          Offsets::IGameObjectExtension,
          64> {
public:
    inline static constexpr auto RTTI = Offsets::RTTI_C_DialogueHolder;

    ~C_DialogueHolder() override;                                      // [0] 0x18116D748
    ...
    bool GetEntityPoolSignature(TSerialize signature) override;       // [13] 0x1829E104C
    ...
    const void* GetRMIBase() const override;                           // [26] 0x181AB4BF0
    void PostUpdate(float frameTime) override;                         // [27] nullsub_1
    void PostRemoteSpawn() override;                                   // [28] nullsub_1

    RTTR_ENABLE() // [29..31]
};
static_assert(sizeof(C_DialogueHolder) == 0x40,
              "C_DialogueHolder must be 0x40");
