...
class C_WaterPuddle
    : public Offsets::CGameObjectExtensionHelper<
          ...
          64> {
public:
    inline static constexpr auto RTTI = Offsets::RTTI_C_WaterPuddle;

    ~C_WaterPuddle() override;                                         // [0] 0x182ACB50C
    ...
    void PostInit(Offsets::IGameObject* gameObject) override;          // [8] 0x182ACD5F4
    ...
    const void* GetRMIBase() const override;                           // [26] 0x181A8B840
    void PostUpdate(float frameTime) override;                         // [27] nullsub
    void PostRemoteSpawn() override;                                   // [28] nullsub
};
static_assert(sizeof(C_WaterPuddle) == 0x40,
              "C_WaterPuddle must be 0x40");
