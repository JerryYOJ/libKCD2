...
class C_LevelHolder
    : public Offsets::CGameObjectExtensionHelper<
          ...
          64> {
public:
    inline static constexpr auto RTTI = Offsets::RTTI_C_LevelHolder;

    ~C_LevelHolder() override;                                         // [0] 0x18127F6BC
    ...
    bool Init(Offsets::IGameObject* gameObject) override;              // [7] 0x18127F304
    ...
    bool GetEntityPoolSignature(TSerialize signature) override;       // [13] 0x18295E448
    ...
    const void* GetRMIBase() const override;                           // [26] 0x181AB4C40
    void PostUpdate(float frameTime) override;                         // [27] nullsub_1
    void PostRemoteSpawn() override;                                   // [28] nullsub_1

    RTTR_ENABLE() // [29..31]

    std::int32_t m_levelId; // +0x40, property "iLevelId"
};
static_assert(sizeof(C_LevelHolder) == 0x48,
              "C_LevelHolder must be 0x48");
static_assert(offsetof(C_LevelHolder, m_levelId) == 0x40,
              "level ID must be at 0x40");
