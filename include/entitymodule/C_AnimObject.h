...
class C_AnimObject
    : public Offsets::CGameObjectExtensionHelper<
          ...
          64> {
public:
    inline static constexpr auto RTTI = Offsets::RTTI_C_AnimObject;

    ~C_AnimObject() override;                                         // [0] 0x18116D748
    ...
    const void* GetRMIBase() const override;                          // [26] 0x181AB4BA0
    void PostUpdate(float frameTime) override;                        // [27] nullsub
    void PostRemoteSpawn() override;                                  // [28] nullsub
};
static_assert(sizeof(C_AnimObject) == 0x40,
              "C_AnimObject must be 0x40");
