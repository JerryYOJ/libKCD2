...
class C_RandomEventPlace
    : public Offsets::CGameObjectExtensionHelper<
          ...
          64> {
public:
    inline static constexpr auto RTTI = Offsets::RTTI_C_RandomEventPlace;

    ~C_RandomEventPlace() override;                                    // [0] 0x18116D748
    ...
    bool GetEntityPoolSignature(TSerialize signature) override;       // [13] 0x182AA7270
    ...
    const void* GetRMIBase() const override;                           // [26] 0x181AB4C90
    void PostUpdate(float frameTime) override;                         // [27] nullsub_1
    void PostRemoteSpawn() override;                                   // [28] nullsub_1
};
static_assert(sizeof(C_RandomEventPlace) == 0x40,
              "C_RandomEventPlace must be 0x40");
