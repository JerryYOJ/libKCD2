...
namespace wh::entitymodule { class C_CartPlayerControl; class C_PlayerExactPosHelper; class C_PlayerMovementAction; }
...
// forward decls for not-yet-RE'd pointee types (stage-2 auto)
class S_PlayerBackPtr;
class S_PlayerBrainVarBinder;
class S_PlayerContext150Helper;
class S_PlayerDogObjectiveHelper;
class S_PlayerMultiSubscribeHelper;
class S_PlayerStimulusToken;
...
public:
    ...
    uint32_t m_trackedEntityId; // +0xB18  (ctor 0)
    ...
    _smart_ptr<wh::entitymodule::C_PlayerMovementAction> m_pMovementAction; // +0xB90  owning smart-ptr (ctor 0; member dtor sub_180DA753C: *this -> Release sub_1804B2F14); pointee class UNRESOLVED
    wh::entitymodule::C_PlayerExactPosHelper* m_pExactPosHelper; // +0xB98  (ctor 0)
    ...
    S_PlayerStimulusToken* m_pStimulusToken; // +0xCB8  (ctor 0)
    uint64_t m_unkCC0;                                 // +0xCC0  (ctor 0)
    void* m_pHandlerCC8; // +0xCC8  (ctor 0)
    uint64_t m_unkCD0;                                 // +0xCD0  (ctor 0)
    uint32_t m_unkCD8;                                 // +0xCD8  (ctor 0)
    float    m_floatCDC;                               // +0xCDC  init 1.0
    S_PlayerDogObjectiveHelper* m_pDogObjectiveHelper; // +0xCE0  owns 0x28 POD {C_Player*@0, u32@8, u8@0xC, u32@0x10, CTimeValue@0x18=-100000, u8@0x20=dog-picker mode (1=SearchItem submit)}; alloc sub_180BC750C, dtor frees size 0x28; pointee unnamed (no vtable)
    ...
    S_PlayerBackPtr* m_pSoulEvent3Listener; // +0xD18  owns 8B {C_Player*@0}; shared factory sub_18099D100 (identical to D28/D30); no vtable; dtor frees size 8
    C_PlayerCarryCorpse* m_pCarryCorpse;               // +0xD20  C_Ptr (0x18)  VERIFIED (NEW in KCD2)
    S_PlayerBackPtr* m_pSoulEvent12Listener; // +0xD28  owns 8B {C_Player*@0}; shared factory sub_18099D100 (identical to D18/D30); no vtable; dtor frees size 8
    S_PlayerBackPtr* m_pSoulEvent4Listener; // +0xD30  owns 8B {C_Player*@0}; shared factory sub_18099D100 (identical to D18/D28); no vtable; dtor frees size 8
    wh::entitymodule::C_CartPlayerControl* m_pCartPlayerControl; // +0xD38  owning unique-ptr (ctor 0; member dtor sub_180DA7030: destroy sub_180DA6D24 then free sub_181AB5160); pointee class UNRESOLVED
    S_PlayerMultiSubscribeHelper* m_pMultiSubscribeHelper; // +0xD40  owns 0x10 {C_Player*@0, u8@8=0}; factory sub_180BC87EC; no vtable; dtor frees size 0x10
    C_PlayerUnconscious* m_pPlayerUnconscious;         // +0xD48  C_Ptr (0x30)  VERIFIED (NEW in KCD2)
    S_PlayerBrainVarBinder* m_pBrainVarBinder; // +0xD50  owns 0x20 {C_Player*@0, u8@8, 4 subscription handles@0xC/0x10/0x14/0x18}; ctor sub_180BC88D4 binds brain-vars "player_armorload","player_hit_side","is_master_strike","player_bushman"
    S_PlayerContext150Helper* m_pContext150Helper; // +0xD58  owns 0x10 {C_Player*@0, u64@8=0}; factory sub_180BC877C; no vtable; ctor/dtor release via cleanup sub_180BC6A64
